...
function getContent (page) {
  return page.evaluate(function () {
    ...
    if (h) {
      header = header || {}
      header.default = h
      header.height = getHeight(document, 'pageHeader')
    }
    removeEl(document, 'pageHeader')

    if (f) {
      footer = footer || {}
      footer.default = f
      footer.height = getHeight(document, 'pageFooter')
    }
    removeEl(document, 'pageFooter')
...
function definePaperSize (content, options) {
  ...
  if (paper.header && paper.header.height === undefined) paper.header.height = paper.header.elHeight + 'px'
  if (paper.footer && paper.footer.height === undefined) paper.footer.height = paper.footer.elHeight + 'px'
